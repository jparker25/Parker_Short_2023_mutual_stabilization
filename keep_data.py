...
def percent_keep(data, percent, direc):
    ...
    check_direc(
        direc
    )  # Check if directory exists, if not create it, for HR integration
    np.savetxt(
        f"{direc}/hr_time_series.txt",
        data,
        delimiter="\t",
        newline="\n",
        header="t x y z data from HR integration",
    )  # Save all the data
    plot_3d_xyz(
        data[int((1 - percent / 100) * (data.shape[0])) :, :], direc
    )  # Plot percent of data in 3D
...
def plot_3d_xyz(data, direc):
    ...
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_zlabel(r"$z$")  # Set x y z labels
