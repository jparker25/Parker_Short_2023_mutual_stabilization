...
def find_cupolets(ctrl, bin_rn_direc):
    ps0micro = np.loadtxt(
        "{0}/microcontrol/ps0_microcontrol.txt".format(bin_rn_direc)
    )  # Read in ps0micro control map
    ps1micro = np.loadtxt(
        "{0}/microcontrol/ps1_microcontrol.txt".format(bin_rn_direc)
    )  # Read in ps1micro control map
    ps0macro = np.loadtxt(
        "{0}/macrocontrol/ps0_macrocontrol.txt".format(bin_rn_direc)
    )  # Read in ps0macro control map
    ps1macro = np.loadtxt(
        "{0}/macrocontrol/ps1_macrocontrol.txt".format(bin_rn_direc)
    )  # Read in ps1macro control map
    found = 0
    found_cupolets = []
    visitations = []
    for i in range(
        ...
        while (
            ...
            if ctrl[ci] == 0:  # If the current control is a 0, apply a microcontrol
                if (
                    ...
                    cbins.append(
                        [int(ps1micro[cbins[-1][0]][0]), int(ps1micro[cbins[-1][0]][1])]
                    )
                    visit.append(1)
                    checks += 1
                else:  # If the current bin is on PS0 apply PS0 micrcontrol
                    cbins.append(
                        [int(ps0micro[cbins[-1][0]][0]), int(ps0micro[cbins[-1][0]][1])]
                    )
                    visit.append(0)
            else:  # If the curret control is a 1, apply a macrocontrol
                if (
                    cbins[-1][1] == 1
                ):  # If the current bin is on PS1 apply PS1 macrcontrol
            ...
            if cbins[0] == cbins[-1] and ci == len(ctrl) - 1:
                ...
                visitations.append(visit)
            ci = 0 if ci == len(ctrl) - 1 else ci + 1
    return found_cupolets, visitations


def find_all_cupolets(
    bits,
    neuron,
    Nfinal,
    bin_rn_direc,
...
def generate_found_cupolets(
    ...
    for cup in found_cupolets:
        ...
        np.savetxt(
            f"{cup_direc}/cupolet_time_series.txt",
            cup[int((1 - cupolet_percent / 100) * cup.shape[0]) :],
            delimiter="\t",
            newline="\n",
    ...
    for i in range(0, Nfinal - 1):
        ...
        if cp.crossed(
            ...
            tp, xp, zp, yp = rk4.rk4_henon(
                np.array([cupolet[i, 0], curr[0], curr[2]]),
                curr[1],
                -(curr[1] - ps1y[0]),
            ...
            # print(ctrl1[ii])
            newval = (
                ps1inits[ii] if control[ctrli] == 0 else ps1inits[int(ctrl1[ii][0])]
            )
            ...
            tp, yp, zp, xp = rk4.rk4_henon(
                np.array([cupolet[i, 0], curr[1], curr[2]]),
                curr[0],
                -(curr[0] - ps0x[0]),
            ...
            newval = (
                ps0inits[ii] if control[ctrli] == 0 else ps0inits[int(ctrl0[ii][0])]
            )
            newdt = dt - (tp - cupolet[i, 0])
            next = rk4.rk4(newval, newdt, neuron.hr_dynamics)
            cupolet[i + 1, 1:] = next
