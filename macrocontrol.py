...
def establish_macrocontrol(direc, coding_fcn=16):
    ...
    store_direc = "{0}/macrocontrol".format(
        direc
    ...
    np.savetxt(
        f"{store_direc}/ps0_macrocontrol.txt",
        ctrl0,
        delimiter="\t",
        newline="\n",
        header="PS0 macrocontrol map.",
    )  # Save the macrocontrol map for PS0
    np.savetxt(
        f"{store_direc}/ps1_macrocontrol.txt",
        ctrl1,
        delimiter="\t",
        newline="\n",
        header="PS1 macrocontrol map.",
    )  # Save the macrocontrol map for PS1
    np.savetxt(
        f"{store_direc}/ps0_diff_macrocontrol.txt",
        diff0,
        delimiter="\t",
        newline="\n",
        header="PS0 macrocontrol difference map.",
    )  # Save the macrocontrol difference map for PS0
    np.savetxt(
        f"{store_direc}/ps1_diff_macrocontrol.txt",
        diff1,
        delimiter="\t",
        newline="\n",
        header="PS1 macrocontrol difference map.",
    )  # Save the macrocontrol difference map for PS1
...
def macro_control_map(rn):
    ...
    ctrl = np.zeros(bins)
    diff = np.zeros(bins)
    # Create empty arrays for the control map and differences
    for i in range(
        0, bins
        ...
        min_sort = np.where(diffs == min(diffs[np.where(diffs > 0)]))[0]
        # Find the bins that are equal to the smallest difference
        ...
        reps = np.count_nonzero(
            bin_dist == min_diff_sort
        )  # Count the number of smallest bin differences
        if reps == 1:  # If only 1 then store that as the nearest value
            ...
            new = np.where(abs(min_sort - i) == min(bin_dist))[
                0
            ]  # Grab the first bin that is the closest distance
...
def macro_control_graphics(ctrl, diff, ps, direc):
    ...
    ax2.plot(
        list(range(0, len(ctrl))), diff, lw=0.5
    )  # # Plot the differences of the control map
    ax2.set_xlabel("Bin")
    ax2.set_ylabel(
        r"Macrocontrol $r_N$ Difference"
    )  # Set tye x and y label of the difference map
    plt.suptitle(
        r"PS{0} Macrocontrol Bin Displacement (Top) and $r_N$ Difference (Bottom)".format(
            ps
        )
    )  # Title the figure
