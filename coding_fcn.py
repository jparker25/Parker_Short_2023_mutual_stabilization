...
def coding_fcn(neuron, direc, bin_rn_direc, dt=1 / 128, coding_fcn_N=16, bins=1600):
    ...
    store_direc = "{0}/coding_fcn".format(
        bin_rn_direc
    ...
    ps1x, ps1y, ps1z = np.loadtxt(
        "{0}/control_planes/ps1_vertices.txt".format(direc), unpack=True
    )
    ps0x, ps0y, ps0z = np.loadtxt(
        "{0}/control_planes/ps0_vertices.txt".format(direc), unpack=True
    )
    ...
    np.savetxt(
        f"{store_direc}/ps1_poly_coeffs.txt",
        ps1_p,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_poly_coeffs.txt",
        ps0_p,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_bin_endpoints.txt",
        bin0,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps1_bin_endpoints.txt",
        bin1,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_bin_midpoints.txt",
        bin0_mids,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps1_bin_midpoints.txt",
        bin1_mids,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_bin_inits.txt",
        bin0_inits,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps1_bin_inits.txt",
        bin1_inits,
        delimiter="\t",
        newline="\n",
    ...
    rn1, map1 = gen_crossing_sequence(
        bin1_inits, neuron, ps0x, ps0y, ps1x, ps1y, N=coding_fcn_N, bins=bins, dt=dt
    )
    rn0, map0 = gen_crossing_sequence(
        bin0_inits, neuron, ps0x, ps0y, ps1x, ps1y, N=coding_fcn_N, bins=bins, dt=dt
    )

    # Save binary decmial for each coding map for each control plane
    np.savetxt(
        f"{store_direc}/ps1_rn_fcn.txt",
        rn1,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps1_rn_map_fcn.txt",
        map1,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_rn_map_fcn.txt",
        map0,
        delimiter="\t",
        newline="\n",
        header="Symbolic future crossings of PS1 for each bin.",
    ...
    plot_rn(
        rn1, rn0, store_direc, bins=bins, coding_fcn_N=coding_fcn_N
    )  # Plto each coding function
    print(f"Saved coding function for each control plane to {store_direc}")


def gen_crossing_sequence(
    x, neuron, ps0x, ps0y, ps1x, ps1y, dt=1 / 128, N=16, bins=1600
):
    """
    ...
    map = np.zeros(
        (bins, N)
    )  # Will contain the binary sequence of the next N control planes
...
def find_fit(ind, dep, degree):
    ...
    """
    psp, resid, _, _, _ = np.polyfit(
        ind, dep, degree, full=True
    )  # Call polyfit function to find coefficients of polynomial degree
    if (
        degree == 3
    ):  # Return coefficients, squared residual fit and polynomical function for degree 3 polynomial
        return psp, np.dot(resid, resid), np.poly1d([psp[0], psp[1], psp[2], psp[3]])
    if (
        degree == 2
    ):  # Return coefficients, squared residual fit and polynomical function for degree 2 polynomial
...
def gen_bin_data(psx, psy, poly, bins, ps):
    ...
    if ps == 0:  # Create all the bin values for PS0
        ...
        mid_db = (bin_endpts[1] - bin_endpts[0]) / 2
        # Half the distance between two bins
        bin_mids = np.linspace(
            bin_endpts[0] + mid_db, bin_endpts[-1] - mid_db, bins
        )  # All the middles of the bins
        bin_inits = np.transpose(
            np.array([psx[0] * np.ones(bins), bin_mids, poly(bin_mids)])
        )  # Initial conditions (x y z) corresponding to each bin
        return (
            bin_endpts,
            bin_mids,
            bin_inits,
        )  # return the endpoints, middles, and initial conditions
    if ps == 1:  # Create all the bin values for PS1
        ...
        bin_inits = np.transpose(
            np.array([bin_mids, psy[0] * np.ones(bins), poly(bin_mids)])
        )  # Initial conditions (x y z) corresponding to each bin
        return (
            bin_endpts,
            bin_mids,
...
def plot_poly_fits(
    bins, ps1pts, ps0pts, bin1, bin0, polyfit1, polyfit0, resid1, resid0, direc
):
    """
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(8, 6), dpi=300
    )  # Create figure to plot polynomial fits
    ...
    ax1.set_xlabel(r"$y$")
    ax1.set_ylabel(r"$z$")  # Label the axes
    ax2.scatter(
        ps1pts[:, 1], ps1pts[:, 3], s=2, color="purple"
    )  # Plot points on PS1 plane
    ax2.plot(bin1, polyfit1, linewidth=0.5)  # Plot the polynomial fit of PS1
    ax2.set_xlabel(r"$x$")
    ax2.set_ylabel(r"$z$")  # Label the axes
    plt.suptitle(
        f"PS0 (top) Resid Sq: {resid0:.2e}, PS1 (bottom) Resid Sq: {resid1:.2e}"
    )  # Title the plot
...
def plot_rn(rn1, rn0, direc, bins=1600, coding_fcn_N=16):
    ...
    fig, (ax1, ax2) = plt.subplots(
        2, 1, figsize=(8, 6), dpi=300
    )  # Create the figure to plot the coding functions
    ax1.plot(list(range(bins)), rn0, lw=0.5)
    ax1.set_xlabel("Bin")
    ax1.set_ylabel(r"$r_N(x)$")
    # Plot the coding function and label the axes for PS0
    ax2.plot(list(range(bins)), rn1, lw=0.5)
    ax2.set_xlabel("Bin")
    ax2.set_ylabel(r"$r_N(x)$")
    # Plot the coding function and label the axes for PS1
    plt.suptitle(
        f"PS0 (top), PS1 (bottom) r(x) - {coding_fcn_N} Crossings, {bins} Bins"
    )  # Title the figure
