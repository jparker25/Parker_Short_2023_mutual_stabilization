        ...
        for neural_i in range(1, neurons):
            if gc.crossed(
                ...
                if (
                    ...
                    neural_link = (
                        neural_i + 1 if neural_i == neurons - 2 else neural_i - 1
                    )
                    newval = (
                        ps1inits[ii]
                        if integrateAndFire(vsms[neural_link][-ifm:], ifm, ifn) == 0
                        else ps1inits[int(ctrl1[ii])]
                    )
                    control[neural_i].append(
                        integrateAndFire(vsms[neural_link][-ifm:], ifm, ifn)
                    )
                    newdt = dt - (tp - sol[i, 0])
                    next = rk4.rk4(newval, newdt, neuron.hr_dynamics)
                    sol[i + 1, 3 * neural_i + 1 : 3 * neural_i + 4] = next
...
def generate_figure_6(
    ...
    sol, vs, vsms, control = unidirectional_chain(
