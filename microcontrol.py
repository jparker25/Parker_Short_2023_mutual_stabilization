...
def establish_microcontrol(neuron, direc, bins_rn_direc, bins=1600, dt=1 / 128):
    ...
    store_direc = "{0}/microcontrol".format(
        bins_rn_direc
    ...
    np.savetxt(
        f"{store_direc}/ps1_microcontrol.txt",
        ps1_micro,
        delimiter="\t",
        newline="\n",
    ...
    np.savetxt(
        f"{store_direc}/ps0_microcontrol.txt",
        ps0_micro,
        delimiter="\t",
        newline="\n",
...
def micro_control_map(neuron, dt, bins, ps, direc, bins_rn_direc):
    ...
    for i in range(0, bins):  # Iterate through each bin and find the microcontrol map
        curr = ps1inits[i] if ps == 1 else ps0inits[i]
