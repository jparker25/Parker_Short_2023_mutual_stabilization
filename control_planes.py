...
def find_surfaces(neuron, dt=1 / 128, percent=75, direc="save_direc"):
    ...
    store_direc = "{0}/control_planes".format(
        direc
    ...
    t, x, y, z = np.loadtxt(
        "{0}/hr_system/hr_time_series.txt".format(direc), unpack=True
    )  # Read in data
    ...
    mid_peaks, _ = find_peaks(
        -x, height=(0, 1)
    )  # Finds indices of -x time series b/w 0 and 1 - used for PS0
    ...
    pts = ps_points(
        t, x, y, z, verts0, verts1, neuron
    )  # Find all the points on the control planes

    # These lines are used to shrink PS0 to a better fit for the points on the plane
    yp0 = [
        p[2] for p in pts if p[4] == 0
    ]  # Grab all the y values for the PS0 points on plane
    ypmax = max(yp0)
    ypmin = min(yp0)
    # Find the min and max of the y values
    ypinc = abs(ypmax - ypmin) * 0.50
    # Slightly extend the y range by 50%
    verts0[0, 1] = ypmin - ypinc
    verts0[1, 1] = ypmax + ypinc
    # Replace y values of PS0 with augmented ones

    np.savetxt(
        f"{store_direc}/ps0_vertices.txt",
        verts0,
        delimiter="\t",
        newline="\n",
        header="X Y Z limits of PS0 plane",
    )  # save PS0 data
    np.savetxt(
        f"{store_direc}/ps1_vertices.txt",
        verts1,
        delimiter="\t",
        newline="\n",
        header="X Y Z limits of PS1 plane",
    )  # Save PS1 data

    np.savetxt(
        f"{store_direc}/ps0_pts.txt",
        np.asarray([pt for pt in pts if pt[4] == 0]),
        delimiter="\t",
        newline="\n",
        header="PS0 points on plane.",
    )  # Save all the PS0 points
    np.savetxt(
        f"{store_direc}/ps1_pts.txt",
        np.asarray([pt for pt in pts if pt[4] == 1]),
        delimiter="\t",
        newline="\n",
        header="PS1 points on plane.",
    )  # Save all the PS1 points
    np.savetxt(
        f"{store_direc}/ps_all_pts.txt",
        pts,
        delimiter="\t",
        newline="\n",
        header="All points on control planes.",
    )  # Save all the PS0 and PS1 points

    generate_figure_ps_pts(
        t, x, y, z, store_direc, pts, verts0, verts1, True
    )  # Create figure of HR neuron with Poincare surfaces and points
    generate_figure_ps_pts(
        t, x, y, z, store_direc, pts, verts0, verts1, False
    )  # Create figure of HR neuron with Poincare surfaces and no points

    print(
        f"Completed integration of Poincare surfaces and established control planes. Saved to {direc}/control_planes"
    )
...
def ps_points(t, x, y, z, verts0, verts1, neuron):
    ...
    for i in range(0, len(x) - 1):  # Iterate through all points
        xn = x[i]
        xn1 = x[i + 1]
        # Grab consecutive x points
        yn = y[i]
        yn1 = y[i + 1]
        # Grab consecutive y points
        zn = z[i]
        zn1 = z[i + 1]
        # Grab consecutive z points
...
def crossed(x_n, x_n1, y_n, y_n1, ps, verts):
    ...
    if ps == 0:  # Check for crossing PS1
        sp = x_n - verts[0, 0]
        sp1 = x_n1 - verts[0, 0]
        # Change of signs in x direction indicates crossing PS1
        if (
            y_n <= verts[1, 1]
            and y_n1 >= verts[0, 1]
            and np.sign(sp1) != np.sign(sp)
...
def generate_figure_ps_pts(t, x, y, z, direc, pts, verts0, verts1, plot_pts):
    ...
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_zlabel(r"$z$")  # Set figure labels
    ax.set_xlim([min(x), max(x)])
    ax.set_ylim([min(y), max(y)])
    ax.set_zlim([min(z), max(z)])  # Set axes limits
    plt.title(
        "PS0 (Yellow), PS1 (Purple) Control Planes and Crossings"
        if plot_pts
        else "Control Planes on HR Neuron"
    )  # Write title
    plt.savefig(
        f"{direc}/ps0_ps1_surfaces_crossings_pts_{plot_pts}.pdf"
    )  # Save the figure
