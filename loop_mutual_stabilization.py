...
def unidirectional_loop(direc,dt,tf,neurons,control1,ifm,ifn):
    ...
    for i in range(0,sol.shape[0]-1):
        ...
        if gc.crossed(sol[i,1],sol[i+1,1],sol[i,2],sol[i+1,2],1,np.array([[ps1x[0],ps1y[0]],[ps1x[1],ps1y[1]]])) and i < 2*neurons*N:
        ...
        elif gc.crossed(sol[i,1],sol[i+1,1],sol[i,2],sol[i+1,2],1,np.array([[ps1x[0],ps1y[0]],[ps1x[1],ps1y[1]]])):
            tp,xp,zp,yp = rk4.rk4_henon(np.array([sol[i,0],sol[i,1],sol[i,3]]),sol[i,2],-(sol[i,2]-ps1y[0]),neuron.hr_dy_dynamics)
            ii = bisect.bisect(ps1endpts,xp)-1;
            newval = ps1inits[ii] if integrateAndFire(vsms[-1][-ifm:],ifm,ifn) == 0 else ps1inits[int(ctrl1[ii])];
            ...
            tp,yp,zp,xp = rk4.rk4_henon(np.array([sol[i,0],sol[i,2],sol[i,3]]),sol[i,1],-(sol[i,1]-ps0x[0]),neuron.hr_dx_dynamics)
            ii = bisect.bisect(ps0endpts,yp)-1;
            newval = ps0inits[ii] if integrateAndFire(vsms[-1][-ifm:],ifm,ifn) == 0 else ps0inits[int(ctrl0[ii])];
            control[0].append(integrateAndFire(vsms[-1][-ifm:],ifm,ifn))
            newdt = dt-(tp-sol[i,0]);
            next = rk4.rk4(newval,newdt,neuron.hr_dynamics)
            sol[i+1,1:4] = next;
...
def plot_raster(sol,neurons,save_direc,dt):
    ...
    t = sol[:,0]
    pks1,_ = find_peaks(sol[:,1],height=1)
    pks2,_ = find_peaks(sol[:,4],height=1)
    pks3,_ = find_peaks(sol[:,7],height=1)
    pks4,_ = find_peaks(sol[:,10],height=1)
    plt.eventplot([t[pks4],t[pks3],t[pks2],t[pks1]],linewidth=0.25,linelengths=0.15)
    plt.yticks(ticks=list(range(neurons)),labels=['Neuron 4','Neuron 3','Neuron 2','Neuron 1'])
    plt.show()
    '''
...
kd.check_direc(save_direc)
sol, vs, vsms, control = unidirectional_loop(direc,dt,tf,neurons,control1,ifm,ifn)
#data_direc = '/Volumes/JEP_IAM/NeuralNetwork/HindmarshRose/cupolets/standard_naming_tests/bins_1600_rN_16/loop_mutual_stabilization/C11/neurons_4_q_4_k_3'
